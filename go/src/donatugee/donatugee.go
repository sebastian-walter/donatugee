...
func (d *Donatugee) Challenges() ([]Challenge, []error) {
	...
	errs := d.db.Preload("Applications").Preload("Techfugees").Find(&challenges).GetErrors()
...
func (d *Donatugee) Challenge(id string) (Challenge, []error) {
	...
	errs := d.db.Preload("Applications").Preload("Techfugees").First(&challenge, "id = ?", newID).GetErrors()
