...
func (d *Donatugee) Challenges() ([]Challenge, []error) {
	...
	errs := d.db.Preload("Applications").Preload("Applications.Techfugee").Find(&challenges).GetErrors()
...
func (d *Donatugee) Challenge(id string) (Challenge, []error) {
	...
	errs := d.db.Preload("Applications").Preload("Applications.Techfugee").First(&challenge, "id = ?", newID).GetErrors()
